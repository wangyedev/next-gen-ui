...
import { useGennie } from "@/lib/gennie-context";
...
export function QueryInterface() {
  const { ask, isInitialized, error: sdkError, schemas } = useGennie();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await ask(input.trim(), "");
        ...
        <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
          ...
          <div className="flex space-x-2">
            ...
            <Badge variant={isInitialized ? "default" : "destructive"}>
              {isInitialized ? `${schemas.length} Schemas` : "Not Initialized"}
            </Badge>
        ...
        <CardContent className="flex-1 flex flex-col space-y-4">
          {(error || sdkError) && (
            <Alert variant="destructive">
              ...
              <AlertDescription>{error || sdkError}</AlertDescription>
            </Alert>
          )}
